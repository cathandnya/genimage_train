"""SD 1.5 Full Fine-Tune メイン学習スクリプト。

diffusersベースでUNet全体を学習する。12GB VRAMに収めるため
gradient checkpointing, fp16, 8-bit Adam, xformers/SDPA, latentキャッシュを使用。

使用方法:
    python -m accelerate.commands.launch train.py
    python -m accelerate.commands.launch train.py --resume outputs/checkpoints/epoch_0002
"""

import argparse
import json
import math
import sys
from pathlib import Path

import torch
import torch.nn.functional as F
from accelerate import Accelerator
from accelerate.logging import get_logger
from accelerate.utils import set_seed
from diffusers import (
    AutoencoderKL,
    DDPMScheduler,
    StableDiffusionPipeline,
    UNet2DConditionModel,
)
from torch.utils.data import DataLoader
from torch.utils.tensorboard import SummaryWriter
from tqdm import tqdm
from transformers import CLIPTextModel, CLIPTokenizer

from config import (
    PROJECT_ROOT,
    LOCAL_MODEL_DIR,
    PROCESSED_DATA_DIR,
    REG_DATA_DIR,
    CHECKPOINT_DIR,
    SAMPLE_DIR,
    LOG_DIR,
    TRAIN_BATCH_SIZE,
    GRADIENT_ACCUMULATION_STEPS,
    LEARNING_RATE,
    LR_SCHEDULER,
    LR_WARMUP_STEPS,
    OPTIMIZER,
    ADAM_BETA1,
    ADAM_BETA2,
    ADAM_WEIGHT_DECAY,
    MAX_GRAD_NORM,
    MAX_TRAIN_EPOCHS,
    MIXED_PRECISION,
    GRADIENT_CHECKPOINTING,
    USE_XFORMERS,
    CACHE_LATENTS,
    FREEZE_TEXT_ENCODER,
    SAVE_EVERY_N_EPOCHS,
    SAMPLE_EVERY_N_EPOCHS,
    SAMPLE_PROMPTS,
    SAMPLE_NEGATIVE_PROMPT,
    NUM_SAMPLE_IMAGES_PER_PROMPT,
    SAMPLE_NUM_INFERENCE_STEPS,
    SAMPLE_GUIDANCE_SCALE,
    SAMPLE_SEED,
    DATALOADER_NUM_WORKERS,
    REG_RATIO,
)
from dataset import BucketSampler, LatentDataset

logger = get_logger(__name__)

TRAINING_STATE_FILE = "training_state.json"


def parse_args():
    parser = argparse.ArgumentParser(description="SD 1.5 Full Fine-Tune")
    parser.add_argument(
        "--resume",
        type=str,
        default=None,
        help="再開するチェックポイントのパス (例: outputs/checkpoints/epoch_0002)",
    )
    return parser.parse_args()


def create_optimizer(unet, optimizer_name, lr, betas, weight_decay):
    """Optimizerを作成する。"""
    params = list(unet.parameters())

    if optimizer_name == "adamw_8bit":
        try:
            import bitsandbytes as bnb

            return bnb.optim.AdamW8bit(
                params, lr=lr, betas=betas, weight_decay=weight_decay
            )
        except ImportError:
            print("警告: bitsandbytesが見つかりません。通常のAdamWにフォールバックします。")
            optimizer_name = "adamw"

    if optimizer_name == "lion":
        try:
            from lion_pytorch import Lion

            return Lion(params, lr=lr * 0.1, betas=(0.95, 0.98), weight_decay=weight_decay)
        except ImportError:
            print("警告: lion_pytorchが見つかりません。通常のAdamWにフォールバックします。")
            optimizer_name = "adamw"

    return torch.optim.AdamW(params, lr=lr, betas=betas, weight_decay=weight_decay)


def create_lr_scheduler(optimizer, scheduler_name, warmup_steps, total_steps):
    """学習率スケジューラを作成する。"""
    from diffusers.optimization import get_scheduler

    return get_scheduler(
        scheduler_name,
        optimizer=optimizer,
        num_warmup_steps=warmup_steps,
        num_training_steps=total_steps,
    )


def save_checkpoint(
    unet, text_encoder, vae, tokenizer, noise_scheduler, optimizer, lr_scheduler,
    epoch, global_step, output_path: Path,
):
    """diffusers形式チェックポイント + 学習状態を保存する。"""
    output_path.mkdir(parents=True, exist_ok=True)

    # diffusers形式で保存
    pipeline = StableDiffusionPipeline(
        vae=vae,
        text_encoder=text_encoder,
        tokenizer=tokenizer,
        unet=unet,
        scheduler=noise_scheduler,
        safety_checker=None,
        feature_extractor=None,
        requires_safety_checker=False,
    )
    pipeline.save_pretrained(str(output_path))

    # UNetのsafetensors保存
    from safetensors.torch import save_file

    state_dict = unet.state_dict()
    save_file(state_dict, str(output_path / "unet.safetensors"))

    # Optimizer + Scheduler + 学習状態を保存（再開用）
    torch.save(optimizer.state_dict(), str(output_path / "optimizer.pt"))
    torch.save(lr_scheduler.state_dict(), str(output_path / "lr_scheduler.pt"))

    training_state = {
        "epoch": epoch,
        "global_step": global_step,
    }
    with open(output_path / TRAINING_STATE_FILE, "w") as f:
        json.dump(training_state, f)

    print(f"チェックポイント保存: {output_path} (epoch={epoch}, step={global_step})")


def generate_samples(
    vae, text_encoder, unet, tokenizer, noise_scheduler,
    epoch: int, global_step: int, output_dir: Path, writer: SummaryWriter,
):
    """学習中にサンプル画像を生成する。

    VAE/text_encoder/unetのコピーを使い、学習側のdevice/dtypeに影響を与えない。
    """
    import copy

    # VAEだけfloat32のコピーを作る（decode時の型不一致を防ぐ）
    sample_vae = copy.deepcopy(vae).to("cuda", dtype=torch.float32)

    pipeline = StableDiffusionPipeline(
        vae=sample_vae,
        text_encoder=text_encoder,
        tokenizer=tokenizer,
        unet=unet,
        scheduler=noise_scheduler,
        safety_checker=None,
        feature_extractor=None,
        requires_safety_checker=False,
    )
    pipeline.set_progress_bar_config(disable=True)
    try:
        pipeline.enable_xformers_memory_efficient_attention()
    except Exception:
        pass

    sample_dir = output_dir / f"epoch_{epoch:04d}"
    sample_dir.mkdir(parents=True, exist_ok=True)

    generator = torch.Generator(device="cuda").manual_seed(SAMPLE_SEED)

    for i, prompt in enumerate(SAMPLE_PROMPTS):
        for j in range(NUM_SAMPLE_IMAGES_PER_PROMPT):
            with torch.no_grad():
                image = pipeline(
                    prompt=prompt,
                    negative_prompt=SAMPLE_NEGATIVE_PROMPT if prompt else "",
                    num_inference_steps=SAMPLE_NUM_INFERENCE_STEPS,
                    guidance_scale=SAMPLE_GUIDANCE_SCALE if prompt else 1.0,
                    generator=generator,
                ).images[0]

            filename = f"prompt{i:02d}_{j:02d}.png"
            image.save(sample_dir / filename)

            # TensorBoardにも記録
            import torchvision.transforms.functional as TF

            img_tensor = TF.to_tensor(image)
            writer.add_image(
                f"samples/prompt_{i}", img_tensor, global_step=global_step
            )

    del sample_vae, pipeline
    torch.cuda.empty_cache()
    print(f"サンプル画像生成: {sample_dir}")


def main():
    args = parse_args()

    # === Accelerator初期化 ===
    accelerator = Accelerator(
        gradient_accumulation_steps=GRADIENT_ACCUMULATION_STEPS,
        mixed_precision=MIXED_PRECISION,
    )

    set_seed(42)

    model_path = str(PROJECT_ROOT / LOCAL_MODEL_DIR)

    # === 再開チェックポイントの解決 ===
    resume_path = None
    start_epoch = 0
    start_global_step = 0

    if args.resume:
        resume_path = Path(args.resume)
        if not resume_path.is_absolute():
            resume_path = PROJECT_ROOT / resume_path
        if not resume_path.exists():
            print(f"エラー: チェックポイントが見つかりません: {resume_path}")
            sys.exit(1)
        state_file = resume_path / TRAINING_STATE_FILE
        if state_file.exists():
            with open(state_file) as f:
                state = json.load(f)
            start_epoch = state["epoch"]
            start_global_step = state["global_step"]
            print(f"再開: epoch {start_epoch}, step {start_global_step} から")
        else:
            print(f"警告: {TRAINING_STATE_FILE} が見つかりません。UNetのみ復元します。")
    else:
        # --resume なしで最新チェックポイントを自動検出
        ckpt_base = PROJECT_ROOT / CHECKPOINT_DIR
        if ckpt_base.exists():
            ckpts = sorted([d for d in ckpt_base.iterdir() if d.is_dir() and (d / TRAINING_STATE_FILE).exists()])
            if ckpts:
                resume_path = ckpts[-1]
                with open(resume_path / TRAINING_STATE_FILE) as f:
                    state = json.load(f)
                start_epoch = state["epoch"]
                start_global_step = state["global_step"]
                print(f"最新チェックポイントを自動検出: {resume_path.name}")
                print(f"再開: epoch {start_epoch}, step {start_global_step} から")

    # === モデル読み込み ===
    print("モデルを読み込み中...")
    tokenizer = CLIPTokenizer.from_pretrained(model_path, subfolder="tokenizer")
    text_encoder = CLIPTextModel.from_pretrained(model_path, subfolder="text_encoder")
    vae = AutoencoderKL.from_pretrained(model_path, subfolder="vae")
    noise_scheduler = DDPMScheduler.from_pretrained(model_path, subfolder="scheduler")

    # UNet: チェックポイントがあればそこから、なければベースモデルから
    if resume_path:
        print(f"UNetをチェックポイントから読み込み: {resume_path}")
        unet = UNet2DConditionModel.from_pretrained(str(resume_path), subfolder="unet")
    else:
        unet = UNet2DConditionModel.from_pretrained(model_path, subfolder="unet")

    # Text encoder凍結
    if FREEZE_TEXT_ENCODER:
        text_encoder.requires_grad_(False)

    # VAE凍結（常に）
    vae.requires_grad_(False)

    # Gradient checkpointing
    if GRADIENT_CHECKPOINTING:
        unet.enable_gradient_checkpointing()

    # xformers / SDPA
    if USE_XFORMERS:
        try:
            unet.enable_xformers_memory_efficient_attention()
            print("xformers有効化")
        except Exception:
            print("xformers無効 → PyTorch SDPAにフォールバック")

    # fp16に変換
    vae.to(dtype=torch.float16)
    text_encoder.to(dtype=torch.float16)

    # === データセット ===
    print("データセットを読み込み中...")
    processed_dir = PROJECT_ROOT / PROCESSED_DATA_DIR
    reg_dir = PROJECT_ROOT / REG_DATA_DIR

    if not processed_dir.exists() or not any(processed_dir.glob("*.pt")):
        print("エラー: キャッシュ済みlatentが見つかりません。")
        print("先に以下を実行してください:")
        print("  python scripts/01_prepare_images.py")
        print("  python scripts/03_cache_latents.py")
        sys.exit(1)

    dataset = LatentDataset(
        data_dir=processed_dir,
        reg_dir=reg_dir if reg_dir.exists() else None,
        tokenizer=tokenizer,
        reg_ratio=REG_RATIO,
    )

    sampler = BucketSampler(dataset, batch_size=TRAIN_BATCH_SIZE, shuffle=True)

    dataloader = DataLoader(
        dataset,
        batch_size=TRAIN_BATCH_SIZE,
        sampler=sampler,
        num_workers=DATALOADER_NUM_WORKERS,
        pin_memory=True,
    )

    # === Optimizer & Scheduler ===
    optimizer = create_optimizer(
        unet, OPTIMIZER, LEARNING_RATE, (ADAM_BETA1, ADAM_BETA2), ADAM_WEIGHT_DECAY
    )

    total_steps = math.ceil(len(dataloader) / GRADIENT_ACCUMULATION_STEPS) * MAX_TRAIN_EPOCHS
    lr_scheduler = create_lr_scheduler(optimizer, LR_SCHEDULER, LR_WARMUP_STEPS, total_steps)

    # === Accelerate準備 ===
    unet, optimizer, dataloader, lr_scheduler = accelerator.prepare(
        unet, optimizer, dataloader, lr_scheduler
    )

    # Optimizer/Schedulerの状態復元
    if resume_path:
        opt_path = resume_path / "optimizer.pt"
        sched_path = resume_path / "lr_scheduler.pt"
        if opt_path.exists():
            print("Optimizer状態を復元中...")
            optimizer.load_state_dict(torch.load(str(opt_path), map_location="cpu", weights_only=True))
        if sched_path.exists():
            print("LR Scheduler状態を復元中...")
            lr_scheduler.load_state_dict(torch.load(str(sched_path), map_location="cpu", weights_only=True))

    text_encoder.to(accelerator.device)
    vae.to(accelerator.device)

    # === TensorBoard ===
    log_dir = PROJECT_ROOT / LOG_DIR
    log_dir.mkdir(parents=True, exist_ok=True)
    writer = SummaryWriter(log_dir=str(log_dir))

    # === 学習ループ ===
    print(f"\n学習開始:")
    print(f"  学習画像: {len(dataset.train_items)}枚")
    print(f"  正則化画像: {len(dataset.reg_items)}枚")
    print(f"  合計: {len(dataset)}枚")
    print(f"  バッチサイズ: {TRAIN_BATCH_SIZE} × 勾配蓄積 {GRADIENT_ACCUMULATION_STEPS}")
    print(f"  エポック: {MAX_TRAIN_EPOCHS}")
    print(f"  総ステップ: {total_steps}")
    print(f"  学習率: {LEARNING_RATE}")
    print(f"  Optimizer: {OPTIMIZER}")
    if start_epoch > 0:
        print(f"  再開: epoch {start_epoch + 1}から (step {start_global_step})")
    print()

    global_step = start_global_step

    for epoch in range(start_epoch, MAX_TRAIN_EPOCHS):
        unet.train()
        epoch_loss = 0.0
        num_batches = 0

        progress = tqdm(dataloader, desc=f"Epoch {epoch + 1}/{MAX_TRAIN_EPOCHS}")

        for batch in progress:
            with accelerator.accumulate(unet):
                latents = batch["latent"].to(dtype=torch.float16)
                input_ids = batch["input_ids"]

                # テキストエンコーディング
                with torch.no_grad():
                    encoder_hidden_states = text_encoder(input_ids)[0]

                # ノイズ追加
                noise = torch.randn_like(latents)
                timesteps = torch.randint(
                    0,
                    noise_scheduler.config.num_train_timesteps,
                    (latents.shape[0],),
                    device=latents.device,
                    dtype=torch.long,
                )
                noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)

                # UNet予測
                noise_pred = unet(
                    noisy_latents, timesteps, encoder_hidden_states
                ).sample

                # Loss
                loss = F.mse_loss(noise_pred.float(), noise.float(), reduction="mean")

                accelerator.backward(loss)

                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(unet.parameters(), MAX_GRAD_NORM)

                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()

            if accelerator.sync_gradients:
                global_step += 1

            epoch_loss += loss.detach().item()
            num_batches += 1

            current_lr = lr_scheduler.get_last_lr()[0]
            progress.set_postfix(loss=f"{loss.item():.4f}", lr=f"{current_lr:.2e}")

            # TensorBoardログ
            writer.add_scalar("train/loss", loss.item(), global_step)
            writer.add_scalar("train/lr", current_lr, global_step)

        avg_loss = epoch_loss / max(num_batches, 1)
        print(f"Epoch {epoch + 1} 完了 - 平均Loss: {avg_loss:.4f}")
        writer.add_scalar("train/epoch_loss", avg_loss, epoch + 1)

        # チェックポイント保存（毎epoch）
        ckpt_dir = PROJECT_ROOT / CHECKPOINT_DIR / f"epoch_{epoch + 1:04d}"
        unwrapped_unet = accelerator.unwrap_model(unet)
        save_checkpoint(
            unwrapped_unet,
            text_encoder,
            vae,
            tokenizer,
            noise_scheduler,
            optimizer,
            lr_scheduler,
            epoch + 1,
            global_step,
            ckpt_dir,
        )

        # サンプル画像生成
        if (epoch + 1) % SAMPLE_EVERY_N_EPOCHS == 0:
            unwrapped_unet = accelerator.unwrap_model(unet)
            sample_output = PROJECT_ROOT / SAMPLE_DIR
            generate_samples(
                vae, text_encoder, unwrapped_unet, tokenizer, noise_scheduler,
                epoch + 1, global_step, sample_output, writer,
            )

    writer.close()
    print("\n学習完了!")
    print(f"チェックポイント: {PROJECT_ROOT / CHECKPOINT_DIR}")
    print(f"サンプル画像: {PROJECT_ROOT / SAMPLE_DIR}")
    print(f"TensorBoardログ: {PROJECT_ROOT / LOG_DIR}")


if __name__ == "__main__":
    main()
